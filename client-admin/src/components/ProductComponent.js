...
class Product extends Component {
  ...
  render() {
    const prods = function name(params) {
      this.state.products.map((item) => {
        return (
          <tr key={item._id} className="database" onClick={() => this.trItemClick(item)}>
            <td>{item._id}</td>
            <td>{item.name}</td>
            <td>{item.price}</td>
    ...
    const panigation = Array.from({length: this.state.noPages}, (_, index) => {
      if ((index + 1) === this.state.curPage) {
        return (<span key={index}>| <b>{index + 1}</b>|</span>);
      } else {
        return (<span key ={ index } className =" link " onClick={() => this.lnkPageClick(index + 1)
        }>| { index + 1}|</span>);
      }
    });
    return (
